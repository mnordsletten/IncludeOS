...
RUN cd ~ && pwd && \
  mkdir -p IncludeOS && \
  mkdir -p IncludeOS/etc
# Copy IncludeOS contents from host repo to container
RUN echo "copying current includeos contents to container"
COPY ./* /root/IncludeOS/

#RUN  cd /root/IncludeOS && echo $(pwd) && \
RUN cp /root/IncludeOS/install_dependencies_linux.sh /root/IncludeOS/etc/install_dependencies_linux.sh
RUN cp /root/IncludeOS/use_clang_version.sh /root/IncludeOS/etc/use_clang_version.sh
RUN cp /root/IncludeOS/use_clang_version.sh /etc/use_clang_version.sh
RUN cp /root/IncludeOS/install_from_bundle.sh /etc/install_from_bundle.sh

RUN cd /root/IncludeOS && ./install.sh -n

#COPY README.md ~/IncludeOS/

# Supports via arguments
#RUN cd ~/IncludeOS
#  git checkout $BRANCH && \
